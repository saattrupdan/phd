...
Showing $(i)\Rightarrow (ii)$ is a forcing argument by an unpublished result of Woodin, and $(ii)\Rightarrow (i)$ is a core model induction by an unpublished result stated in
\cite{Sargsyan}.\todo[inline]{Figure out a polite way to state that Grigor thinks his claimed proof doesn`t work.}

\qquad Most of our results don't require the full strength of $\di^+$ and we've stated the needed assumptions in these cases, but the reader may simply assume $\di^+$ for the remainder of this paper if they wish to.
