...
\defi[Schlutzenberg-Trang]{
  Let $\kappa$ be an infinite cardinal, $D$ a set of self-wellordered\footnote{A set $x$ is \textit{self-wellordered} if there's a wellorder of $x$ in $\J(x)$.} sets and fix $b\in H_\kappa$. An \textbf{operator over $H_\kappa$ on $b$ with support $D$} is a partial function
  \eq{
...
\qdefi{
  Let $\kappa$ be an infinite cardinal and $\F$ an operator over $H_\kappa$ on $b\in H_\kappa$. Then define the \textbf{$\F$-lower part model on $b$} as
  \eq{
...
\defi[Schindler-Steel]{
  Let $\kappa$ be an infinite cardinal and $\F$ an operator over $H_\kappa$ on $b\in H_\kappa$. Then $\F$ \textbf{condenses well} if whenever $g\subset\col(\omega,\kappa)$ is $V$-generic and that there are models $\overline\M, \M\in H_\kappa$ and $\overline\M^+\in V[g]$, all on $b$, with 
  \begin{enumerate}
