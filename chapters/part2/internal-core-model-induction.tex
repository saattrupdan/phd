...
\defi[Schlutzenberg-Trang]{
  For a set $x$ write $\rho_x:x\to\rk x$ for the rank function of $x$, and also define $\hat x:=\trcl(\{x, \rho_x\})$.
}

\defi[Schlutzenberg-Trang]{
  Let $\kappa$ be an infinite cardinal and fix $b\in H_\kappa$. An \textbf{operator over $H_\kappa$ on $b$} is a partial function $\F\colon H_\kappa\to H_\kappa$ where
  \eq{
    C_b := \{\hat x\in H_\kappa\mid b\in\J_1(\hat x)\} \subset \dom\F
  }

  and such that $\dom\F$ is closed under both unions and applications of $\F$. We also call $C_b$ the \textbf{cone over $b$} and $b$ the \textbf{base of $C_b$}.
}
