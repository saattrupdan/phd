...
\lemm[][lemm.opit]{
  ...
  $\theta=\infty$ and let $\N$ be a tame \todo{Define this} hybrid mouse
  ...
  \proof{
    Fix $x\in H_\theta$. We first show
  ...
  $\M^{\overline\T}_{\overline b}$.\todo{Why is that?}
  ...
  \cproof{
    ...
    satisfies $(2)$ of the definition of relativises well\todo{Define this, cutpoint and $\M_b^{\T}$}, meaning
    that
    ...
    first exit extender \todo{Define this} of $\N(\M(\overline\T))$ in the
...
\defi{
  Let $j:V\to M$ be an elementary embedding in some $V[g]$ and let $F$ be a model operator. Then $F$ is \index{Operator!Radiant operator} \textbf{$j$-radiant} if it condenses well, determines itself on generic extensions and satisfies the \index{Extension property}\index{Operator!Extension property of an operator} \textbf{extension property}, which says that $F\subset j(F)$ and $j(F)\restr\hc^{V[g]}$ is definable in $V[g]$.
}
