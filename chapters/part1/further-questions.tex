...
\ques[][ques.prestrong-equivalence]{
  Let $\theta$ be an uncountable cardinal. Is every virtually $\theta$-measurable cardinal also virtually $\theta$-prestrong? How about if we assume that $V=L[\mu]$ or $V=K$ with $K$ being the core model below a woodin cardinal?
}
