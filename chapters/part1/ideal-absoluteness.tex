...
\defi{
  A cardinal $\kappa$ is \textbf{ideally measurable} if there exists an ideal $\I$ on $\theta$ such that the generic ultrapower $\ult(V,\I)$ is well-founded in the forcing extension $V^{\mathbb P}$, where $\mathbb P := \p^V(\kappa)/\I$.
}
...
\proof{
  ...
  Note that $\A^*_\alpha$ is an antichain in $\mathbb P$. They are also maximal, because if $p\in\mathbb P$ was incompatible with every condition in $\A^*_\alpha$ then, letting $X:=\{\kappa - Y\mid Y \in \A_\alpha\}$, we have that $p$ is compatible with $\norm{\check X\in\dot\mu}_{\B(\mathbb P)}$, so that $X\in \I^+$. But $X$ is incompatible with everything in $\A_\alpha$, contradicting that $\A_\alpha$ is maximal.
...
\qquad Now note that $\I^{\eta+1}\subset\I^\eta$ and $\T^\eta\subset\T^{\eta+1}$ for every $\eta<\kappa^+$ --- define the ideal $\I:=\bigcap_{\eta<\kappa^+}\I^\eta$ and the tree $\T:=\bigcup_{\eta<\kappa^+}\T^\eta$. We showed that all hopeless ideals are $\kappa$-complete, normal and $(\kappa,\kappa)$-distributive, so this holds in particular for the $\I^\eta$'s and thus also for $\I$.
...
\qcoro[N.]{
	The property ``${<}\lambda$-closed $\lambda$-sized'' is ideal-absolute for all regular $\lambda\in[\omega,\kappa^+]$.
}
