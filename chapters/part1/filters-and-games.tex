...
\proof{
  Assume $\cof\gamma>\omega$ and fix $\theta_0,\theta_1>\kappa$. First assume that player I has a winning strategy $\sigma_0$ in $\G_\gamma^{\theta_0}(\kappa)$; we'll informally describe a winning strategy $\sigma_1$ for player I in $\G_\gamma^{\theta_1}(\kappa)$. Whenever $\sigma_0$ plays a weak $\kappa$-model $\M_\alpha\prec H_{\theta_0}$ we simply let $\sigma_1$ play
  \eq{
