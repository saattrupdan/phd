...
\lemm[Virtualised Ad\' amek-Rosick\' y; \gbc][lemm.adaros]{
  $\gvp$ is equivalent to there \textit{not} existing an $\on$-sequence of first-order structures $\bra{\M_\alpha\mid\alpha<\on}$ satisfying that\footnote{This is equivalent to saying that $\on$, viewed as a category, cannot be fully embedded into the category \textsf{Gra} of graphs, which is how it is stated in \cite{AdamekRosicky}.}
  \begin{enumerate}
    \item \gvp\
