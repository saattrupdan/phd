...
\prop[N.]{
  ...
  \begin{enumerate}
    ...
    \item $L[\mu]\models\godel{\text{faintly $\theta$-measurables are equivalent to virtually $\theta$-measurable}}$.\footnote{Assuming that $L[\mu]$ exists, of course.}
  \end{enumerate}
}
\proof{
...
Note that the proof of $(ii)$ above does not show that $\kappa$ is virtually $\theta$-prestrong, as it might still be the case that $\bar\mu\neq\mu$, so we cannot conclude that $L_\theta[\mu]\subset L_\theta[\bar\mu]$. It might still hold however; see Question \ref{ques.prestrong-equivalence-lmu}
