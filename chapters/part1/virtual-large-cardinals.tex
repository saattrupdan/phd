...
\qquad But, surprisingly, the following result by Usuba shows that they can exist in $L$.
...
\proof{
  ...
  \cproof{
    ...
    \todo[inline]{A possible fix: Since $\N$ is (in particular) $\abs{\mathbb P}$-closed in $V[h]$ and $\mathbb P$ is trivially $\abs{\mathbb P}^+$-cc, Lemma \ref{lemm.lucke-schlicht} implies that $\N$ is still $\abs{\mathbb P}$-closed in $V[g\times h]$. As below, we thus still get that $\pi[g]\in\N$. The remaining bit of the argument uses full ${<}\theta$-closure, but we can carry that bit out in $V[h]$. This means that $\tilde g\in V[h]$ and in $V[h]$ we thus get $\tilde\pi$ in $V[h\times g]=V[g\times h]$.}
    \todo[inline]{The above argument might not work, say if $\theta$ is collapsed to $\kappa$.}

    Since $\pi(\mathbb P)$ is ${<}\pi(\kappa)$-directed closed in $\N$ and $^{<\theta}\N\subseteq\N$, it is ${<}\theta$-directed closed. Moreover, $\pi[g]\in\N$, since $|\pi[g]|\leq|\mathbb P|<\theta$ and $^{<\theta}\N\subseteq\N$. Since $\pi(\mathbb P)$ is directed, there's a condition $q\leq \pi[g]$ in $\pi(\mathbb P)$. Using the fact that $\abs{\N}=\theta$ and $\pi(\mathbb P)$ is ${<}\theta$-closed, we can construct a $\pi(\mathbb P)$-generic filter $\tilde{g}$ over $\N$ with $q\in g$.\footnote{Namely, enumerate the dense subsets of $\pi(\mathbb P)$ that are elements of $\N$ in order type $\theta$ and use the fact that the initial segments of the sequence, and of the corresponding sequence of conditions that we construct, are in $\N$.} Then $\tilde{g}$ is as required.
  }
...
\proof{
  ...
  \todo[inline]{Missing proof, same subsets of $\kappa$, embeddings are iteration embeddings by Kanamori, small embedding lives in target model}
}
