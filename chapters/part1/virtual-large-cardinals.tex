...
\rema[][rema.levelwise-strong-supercompact]{
  As mentioned above, the proof in fact shows something stronger: if $\kappa$ is virtually $(2^{<\theta})^+$-strong then it is virtually $\theta$-supercompact, and if it is virtually $(2^{<\theta})^+$-Magidor-supercompact then it is virtually $\theta$-supercompact. It is open whether they are equivalent level-by-level (see Question \ref{ques.remarkableequiv}).
}
