...
\todo[inline]{Perhaps properly state the above results, maybe prove them, or just leave them out?}

\qquad Figure \ref{fig.virtual-direct-implications} summarises the separation results along with the core model results from the preceeding chapter.\todo{Explain the arrows. Here, going from the bottom and up, it's (1) precip ideal on $\omega_1$, (2) 2.11, (3) 2.11, (4) ?? and (5) Question 4.3.}

\figx[Relations between virtuals, where a black arrow indicates a direct implication and a red arrow indicates that a direct implication is not provable in $\zfc$.][fig.virtual-direct-implications]{virtual.pdf}

\qquad As a final result of this section, we note that the ``virtually'' adverb \textit{does} yield cardinals different from the faintly ones. This is trivial in general as successor cardinals can be faintly measurable and are never virtually measurable, but the separation still holds true if we rule out this successor case.
