...
\qquad Figure \ref{fig.virtual-separations} summarises the separation results along with the results from Section \ref{section.canonical-inner-models}.\todo{Explain the diagram, possibly also in caption}

\figx[fig.virtual-separations][200]{virtual-separations.pdf}{Direct implications between virtuals.}
...
\qquad Ostensibly this seems to be an incredibly strong notion, as the target model now inherits a lot of structure from the generic extension. But, surprisingly, the following result by Usuba shows that they can exist in $L$.

\todo[inline]{I think that Usuba's result shows that the virtually extendibles are equiconsistent with the generically setwise supercompacts, since the closure of $\N$ should imply that any restriction of the embedding witnesses virtually $\lambda$-extendible for $\lambda<\theta$, so in the limit they catch up.}
...
\todo[inline]{Note that the proof breaks down if $\mathbb P$ had size $\kappa$}
...
\proof{
  ...
  \qquad In $V$ fix a regular $\theta>\kappa$ such that $\mathbb P\in h_\theta$, and let $\mathbb Q$ be the forcing given by the definition of setwise supercompactness. Let $h$ be $\mathbb Q$-generic over $V[g]$ and work in $V[g\times h]$. Let $\pi\colon h_\theta^V\rightarrow N$ be as in the definition of generically setwise supercompactness.
  
  \qquad We may assume that $\theta=\theta^{<\theta}$ holds, as otherwise we just replace $\mathbb Q$ with $\mathbb{Q}*\col(\theta,\theta^{<\theta})$ --- we retain the ${<}\theta$-closure of $N$ because $\col(\theta,\theta^{<\theta})$ is ${<}\theta$-closed. We can further assume that $|N|=\theta^{<\theta}=\theta$, as otherwise we can take a hull of $\N$ containing $\ran(\pi)$ and recursively close under ${<}\theta$-sequences, ending up with a ${<}\theta$-closed elementary substructure $\h\prec\N$ containing $\ran(\pi)$ -- now replace $\N$ by the transitive collapse of $\h$.
  ...
  \cproof{
    \todo[inline]{We maybe need to absorb $g$ into $h$ here? Monroe Eskew noticed this mistake in Philipp's slides in Kyoto, November 2019.}

    Since $\pi(\mathbb P)$ is ${<}\pi(\kappa)$-directed closed in $N$ and $^{<\theta}N\subseteq N$, it is ${<}\theta$-directed closed. Moreover, $\pi[g]\in N$, since $|\pi[g]|\leq|\mathbb P|<\theta$ and $^{<\theta}N\subseteq N$. Since $\pi[g]$ is directed, there's a condition $q\leq \pi[g]$ in $\pi(\mathbb P)$. Using the fact that $|N|=\theta$ and $\pi(\mathbb P)$ is ${<}\theta$-closed, we can construct a $\pi(\mathbb P)$-generic filter $\tilde{g}$ over $N$ with $q\in g$.\footnote{Namely, enumerate that dense subsets of $\pi(\mathbb P)$ that are elements of $N$ in order type $\theta$ and use the fact that the initial segments of the sequence, and of the corresponding sequence of conditions that we construct, are in $N$.} Then $\tilde{g}$ is as required.
  }
...
\todo[inline]{Note that if there exists a proper class of woodin cardinals then $\omega_1$ is generically setwise supercompact, using the countable stationary tower. Daisuke claims that it also holds if we replace $\omega_1$ by any singular cardinal.}
...
\proof{
  Assume first that $V=L$ and that $\kappa$ is generically setwise supercompact. Let $g$ be a generic filter and $\pi\colon\M=L_\theta\rightarrow\N$ an embedding in $V[g]$ with $\pi\restr L_{\kappa^{+L}} \in\N$. Then $\N=L_\alpha$ for some $\alpha$ by condensation and thus $\pi\restr H_{\kappa^{+L}}\in L$. But this would induce a  ${<}\kappa$-complete ultrafilter on $\kappa$, contradicting $V=L$.

  \qquad Assume now that $V=L[\mu]$.
