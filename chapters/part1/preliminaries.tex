...
When we're dealing with elementary embeddings between set-sized structures, we will usually be interested in structures of the following form.
...
\qprop[Kunen]{
	Let $\M$ be a weak $\kappa$-model, $\mu$ an $\M$-measure and $j:~\M\to\N$ the associated ultrapower embedding. Then $\mu$ is weakly amenable iff $j$ is \textbf{$\kappa$-powerset preserving}, meaning that $\M\cap\p(\kappa)=\N\cap\p(\kappa)$.
}
...
\qprop[\cite{Kanamori} Proposition 1.2]{
  If $\kappa$ is an inaccessible cardinal then $(V_\kappa, \in)\models\zfc$.\todo{Who proved this?}
}
...
\qtheo[\cite{Jech} Lemma 9.9]{
  Every weakly compact cardinal is a limit of inaccessible cardinals.\todo{Who proved this?}
}
...
\qprop[\cite{Kanamori} 26.6]{
  Strong cardinals are measurable limits of measurable cardinals.\todo{Who proved this?}
}
...
\theo[\cite{Kanamori} Theorem 26.14]{
  The following are equivalent for an uncountable cardinal $\kappa$.\todo{Who proved this?}
  \begin{enumerate}
...
\todo[inline]{Replace weak covering figure by tikz}
...
\qquad The takeaway here is that whenever we're working with an initial segment of $K$ then that segment will be build using the recursive steps $(i)$ and $(ii)$ above, carefully including extenders from $V$. For more details, see \cite{Kwithoutmeasurable} or \cite{MSc}.
...
\qtheo[Zeman][theo.beaver]{
  Assume $0^\pistol$ does not exist. If $\mu$ is a countably complete weakly amenable $K$-measure then $\mu\in K$.\todo{Check if Zeman is the author}
}
